export default class Crossword {
  ...
  async highlight() {
    ...
    for (let i in this.grid_nums) {
      ...
      if (this.across_nums.includes(this.grid_nums[i])) {
        ...
        // highlight clue
        let acrossClueElem = document.querySelector(
          `.across-clue-${this.grid_nums[i]}`
        );
        acrossClueElem.lastElementChild.style.backgroundColor =
          "var(--secondary-blue)";
        acrossClueElem.firstElementChild.style.backgroundColor =
          "var(--secondary-blue)";
        currentlyHighlightingClue = acrossClueElem;

        // scroll to make the clue visible
        let scrollPosition =
          acrossClueElem.offsetTop -
          acrossCluesDiv.offsetTop -
          acrossCluesDiv.clientHeight / 2;
        acrossCluesDiv.scrollTop = scrollPosition;

        // highlight cells
        let length =
          this.across_length[this.across_nums.indexOf(this.grid_nums[i])];
      ...
      if (this.down_nums.includes(this.grid_nums[i])) {
        ...
        let currentlyHighlightingCellsPos = [];
        // highlight clue
        let downClueElem = document.querySelector(
          `.down-clue-${this.grid_nums[i]}`
        );
        downClueElem.lastElementChild.style.backgroundColor =
          "var(--secondary-blue)";
        downClueElem.firstElementChild.style.backgroundColor =
          "var(--secondary-blue)";
        currentlyHighlightingClue = downClueElem;

        // scroll to make the clue visible
        let scrollPosition =
          downClueElem.offsetTop -
          downCluesDiv.offsetTop -
          downCluesDiv.clientHeight / 2;
        downCluesDiv.scrollTop = scrollPosition;

        // highlight cells
        let length =
          this.down_length[this.down_nums.indexOf(this.grid_nums[i])];
        let rowPos = Math.floor(i / this.dimension);
        let colPos = i % this.dimension;
        for (let j = 0; j < length; j++) {
  ...
  getJSONForSolving() {
    ...
    for (let clue_num of this.across_nums) {
      ...
      acrossAnswers.push(
        "A".repeat(this.across_length[this.across_nums.indexOf(clue_num)])
      );
    }
    for (let clue_num of this.down_nums) {
      ...
      downAnswers.push(
        "A".repeat(this.down_length[this.down_nums.indexOf(clue_num)])
      );
    }
  ...
  rotateGrid(clockwise = false) {
    ...
    for (let i = 0; i < this.dimension; i++) {
      ...
      for (let j = this.dimension - 1; j >= 0; j--) {
        rotatedTempGrid[i].push(
          this.cells[(this.dimension - 1 - j) * this.dimension + i]
        );
      }
    }
    ...
    for (let i = 0; i < this.dimension; i++) {
      for (let j = 0; j < this.dimension; j++) {
        this.cells[i * this.dimension + j].replaceWith(
          rotatedTempGrid[i][j].cloneNode(true)
        ); // replace with new cells having no new event listeners
      }
    }
  ...
  handleCellClick(cell, index) {
    ...
    if (!(click_x === click_y && click_x === Math.floor(this.dimension / 2))) {
      ...
      if (cell.classList.contains("dead-cell")) {
        // toggle if original is dead cell and mirror is not
        if (!cell_mirror.classList.contains("dead-cell")) {
          cell_mirror.classList.toggle("dead-cell");
        }
      } else {
        if (cell_mirror.classList.contains("dead-cell")) {
          // toggle if original is not dead cell and the mirror is
          cell_mirror.classList.toggle("dead-cell");
        }
      }
  ...
  addClueEditEventListener() {
    this.acrossClues.forEach((elem) => {
      elem.lastElementChild.addEventListener("input", () => {
        this.acrossCluesWithNums[elem.firstElementChild.innerText] =
          elem.lastElementChild.innerText.trim();
      });
    });
    this.downClues.forEach((elem) => {
      elem.lastElementChild.addEventListener("input", () => {
        this.downCluesWithNums[elem.firstElementChild.innerText] =
          elem.lastElementChild.innerText.trim();
      });
    });
  }
  ...
  assignNewClues() {
    ...
    this.acrossClues.forEach((elem) => {
      // deleting previous DOM elements
  ...
  computeGridNum() {
    ...
    for (let x = 0; x < this.dimension; x++) {
      for (let y = 0; y < this.dimension; y++) {
        ...
        let horizontal_presence = in_horizontal.some(
          (coord) => coord[0] === x && coord[1] === y
        );
        let vertical_presence = in_vertical.some(
          (coord) => coord[0] === x && coord[1] === y
        );
        ...
        if (!horizontal_presence && vertical_presence) {
          ...
          while (
            x + horizontal_length < this.dimension &&
            this.grid[x + horizontal_length][y] !== "."
          ) {
        ...
        if (!vertical_presence && horizontal_presence) {
          ...
          while (
            y + vertical_length < this.dimension &&
            this.grid[x][y + vertical_length] !== "."
          ) {
